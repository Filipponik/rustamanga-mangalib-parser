...
impl Builder {
    ...
    pub fn user_agent(mut self, user_agent: &str) -> Builder {
        self.user_agent = Some(user_agent.to_string());
        self
    }

    pub fn accept_language(mut self, accept_language: &str) -> Builder {
        self.accept_language = Some(accept_language.to_string());
        self
    }

    pub fn platform(mut self, platform: &str) -> Builder {
        self.platform = Some(platform.to_string());
        self
    }

    pub fn image_server_prefix(mut self, image_server_prefix: &str) -> Builder {
        self.image_server_prefix = Some(image_server_prefix.to_string());
        self
    }

    pub fn base_url(mut self, base_url: &str) -> Builder {
    ...
    pub fn build(self) -> HeadlessBrowserClient
    {
        HeadlessBrowserClient {
            ...
            accept_language: self.accept_language.unwrap_or(ACCEPT_LANGUAGE.to_string()),
            platform: self.platform.unwrap_or(PLATFORM.to_string()),
            image_server_prefix: self.image_server_prefix.unwrap_or(IMAGE_SERVER_PREFIX.to_string()),
            base_url: self.base_url.unwrap_or(MANGALIB_DEFAULT_BASE_URL.to_string()),
        }
    }
}